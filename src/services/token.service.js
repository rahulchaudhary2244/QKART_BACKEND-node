...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
    ...
    // const options = { expiresIn: '1d' };//expires
...
const generateAuthTokens = async (user) => {
  // const token = await generateToken(user.id, config.jwt.accessExpirationMinutes , tokenTypes.ACCESS);
  // //expiryDate = new Date( currentTimeInMilliseconds + expirationTimeInmilliseconds )
  // const expiryDate = new Date((Date.now() + config.jwt.accessExpirationMinutes * 6000))
  // return {
  //   access: {
  //     token: token,
  //     expires: expiryDate,
